...
dunnTest(score ~ grow_up_continent, data = European, method = 'holm')
...
# 
# num_of_lan_df <- recording_df_long %>%
#   mutate(num_of_language = replace(num_of_language, num_of_language == '1', '2'))
# 
# # correlation analysis 
# num_of_lan_df %>% 
#   group_by(speaker_accent) %>%
#   fileter(is.na(num_of_language) == FALSE) %>%
#   summarise(spear_cor = cor.test(score, num_of_language))
# 
# 
# recording_df_long %>%
#   mutate(
...
#   theme_bw()
#   
# # treat num_of_language as numeric  
# mol_num_lang_1 <- recording_df_long %>% 
#   mutate(
#     num_of_language = replace(num_of_language, num_of_language == '+ de 5', '5'),
#     num_of_language = replace(num_of_language, num_of_language == '1', '2'),
#     num_of_language = as.numeric(num_of_language)
...
#   
# for (i in 1:5){
#   print(broom::tidy(mol_num_lang_1$model[[i]]))
# }  
# 
# 
# # treat num_of_language as categorical   
# mol_num_lang_2 <- recording_df_long %>% 
#   group_by(speaker_accent) %>% 
#   do(model = lm(score ~ num_of_language, data = .)) 
# 
#   
# for (i in 1:5){
#   print(broom::tidy(mol_num_lang_2$model[[i]]))
# }  
# 
# 
# # KW Test 
# recording_df_long %>% 
#   group_by(speaker_accent) %>% 
#   kruskal_test(score ~ num_of_language) 
...
rec_long_accent %>%
  ...
  theme_bw() + 
  labs(x = 'Accent', y = 'Mean Score') + 
  scale_colour_discrete('Race of Respondents') + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
