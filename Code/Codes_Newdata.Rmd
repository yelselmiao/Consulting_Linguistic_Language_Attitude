...
formatted_eta_squared <- sprintf("%.2f", eta_squared)
...
ggboxplot(
  ...
  # stat_pvalue_manual(pwc, tip.length = 0, hide.ns = FALSE) +
  labs(
    subtitle  = bquote("Anova, F(" ~ .(df1) ~ "," ~ .(df2) ~ ") =" ~ .(f_value) ~ ", p < " ~ .(format(p_value, scientific = FALSE)) ~ ", " ~ eta[g]^2 ~ " =" ~ .(eta_squared)), 
