...
# live in Quebec, live in Frnace, live in other French-speaking, and none
...
rownames(res_tbl_und) <- c('Easy to Understand', 'Good French Teacher', 'Weak Accent')
res_tbl_und
```
...
# one-sided t-test
t.test(b_df$score, w_df$score, alternative= "greater") 

# one-sided Wilcoxon test
wilcox.test(b_df$score, w_df$score, alternative= "greater")
...
recording_df_sum_con %>% 
  dunn_test(score_sum ~ continent, p.adjust.method = "bonferroni") 
...
```{r continent vs. accent, fig.width=8}
recording_df_long %>%
...
recording_df_long %>%
  ggplot(aes(x=accent, y=score, fill=accent)) +
  geom_boxplot(outlier.colour=NA, position=position_dodge(1)) +
...
```{r continent vs. race, fig.width=6, fig.height=3}
recording_df_long %>%
  ggplot(aes(x=continent, y=score, fill=continent)) +
  geom_boxplot(outlier.colour=NA, position=position_dodge(1)) +
  facet_wrap(~race) + 
...
recording_df_long %>%
  ggplot(aes(x=race, y=score, fill=race)) +
  geom_boxplot(outlier.colour=NA, position=position_dodge(1)) +
  facet_wrap(~continent) + 
  viridis::scale_fill_viridis(discrete=T, name= '') +
  hrbrthemes::theme_ipsum() + 
...
```{r teacher vs. accent, fig.width=8, fig.height= 5}
recording_df_long %>%
  #mutate(teacher_origin=replace(teacher_origin, teacher_origin=='Elsewhere (specify)', 'Elsewhere')) %>%
...
```{r teacher vs. race, fig.width=6, fig.height=3}
recording_df_long %>%
  filter(is.na(teacher_origin) == FALSE) %>%
  ggplot(aes(x=teacher_origin, y=score, fill=teacher_origin)) +
  geom_boxplot(outlier.colour=NA, position=position_dodge(1)) +
...
```{r preference vs. accent, fig.width=8, fig.height= 5}
recording_df_long %>%
...
```{r Preference vs. race, fig.width=6, fig.height=3}
recording_df_long %>%
  filter(is.na(type_of_french) == FALSE) %>%
  ggplot(aes(x=type_of_french, y=score, fill=type_of_french)) +
  geom_boxplot(outlier.colour=NA, position=position_dodge(1)) +
...
```{r}
summary(lm(score ~ continent + accent + continent*accent, data = recording_df_long))

```


```{r}
summary(lm(score ~ continent + accent + race + type_of_french + teacher_origin, data = recording_df_long))
```

```{r}
accent_df <- recording_temp_long_sep %>%
  group_by(id, accent) %>% 
  summarise(score_sum = sum(score)) %>% 
  left_join(recording_df_sum %>% select(id, gender, age, continent,teacher_origin, type_of_french), by = 'id') %>%
  mutate_if(is.character, as.factor) %>%
  ungroup()

summary(lm(score_sum ~ accent + continent, data = accent_df))
...
# accent vs. preferred_type_of_french
accent_df %>%
  drop_na() %>%
  group_by(accent) %>%
...
```{r}
race_df <- recording_temp_long_sep %>%
  group_by(id, race) %>% 
  summarise(score_sum = sum(score)) %>%
  left_join(recording_df_sum %>% select(id, gender, age, continent,teacher_origin, type_of_french), by = 'id') %>%
  mutate_if(is.character, as.factor) %>%
...
# race vs. preferred_type_of_french
race_df %>%
  drop_na() %>%
  group_by(race) %>%
  kruskal_test(score_sum ~ type_of_french) 
...
```{r}
table(recording_df_sum$type_of_french)
...
```{r Two-way repeated measures ANOVA}
twoway.teacher.accent <- anova_test(
  data = recording_df_long, dv = score, wid = id,
  within = c(accent, teacher_origin)
  )
get_anova_table(twoway.teacher.accent)  
```
